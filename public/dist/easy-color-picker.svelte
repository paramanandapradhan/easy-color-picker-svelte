...
export let rgbaFormat = false;
export let copyString = "Copied!";
export let colorPalletes = [];
...
function saveLocalColor(color2) {
...
function pickColor(event) {
  if (!isGradientDragging && !!!event.color)
    return;
...
function pickAlpha(event) {
  if (!isAlphaDragging && !!!event.color)
    return;
  drawAlphaBar();
